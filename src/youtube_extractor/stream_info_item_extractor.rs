...
impl YTStreamInfoItemExtractor<'_>{
    ...
    pub fn is_ad(&self)->bool{
        self.item.select(&Selector::parse("span[class*=\"icon-not-available\"]").unwrap()).next().is_some()
            || self.item.select(&Selector::parse("span[class*=\"yt-badge-ad\"]").unwrap()).next().is_some()
            || self.is_premium_video()
    }

    pub fn is_premium_video(&self)->bool{
        let premium_span = self.item.select(&Selector::parse("span[class=\"standalone-collection-badge-renderer-red-text\"]").unwrap()).next();

        match premium_span {
            None => false,
            Some(premium_span) => {
                premium_span.text().next().is_none()
            }
        }
    }
    ...
    pub fn is_live(&self)->bool{
        self.item.select(&Selector::parse("span[class*=\"yt-badge-live\"]").unwrap()).next().is_some()
            || self.item.select(&Selector::parse("span[class*=\"video-time-overlay-live\"]").unwrap()).next().is_some()
    }
    ...
    pub fn get_duration(&self)->Option<u64>{
        ...
        let el = self.item.select(&Selector::parse("span[class*=\"video-time\"]").unwrap()).next()?;
...
//        let el = el.select(&Selector::parse("a").unwrap()).next()?;
    ...
    pub fn get_textual_upload_date(&self)->Option<&str>{
        ...
        let el = self.item.select(&Selector::parse("div[class*=\"yt-lockup-meta\"] li").unwrap()).next()?;
        el.text().next()
    }

    pub fn get_view_count(&self)->Option<u32>{
        let input:Option<&str> ;
        let span_view_count = self.item.select(&Selector::parse("span.view-count").unwrap()).next();
        if let Some(span_view_count)= span_view_count{
            input = span_view_count.text().next();
        }
        else if self.is_live() {
            let meta = self.item.select(&Selector::parse("ul.yt-lockup-meta-info").unwrap()).next()?;
            input = meta.select(&Selector::parse("li").unwrap()).next()?.text().next()
        }
        else{
            let meta = self.item.select(&Selector::parse("div.yt-lockup-meta").unwrap()).next()?;
            let lis = meta.select(&Selector::parse("li").unwrap()).collect::<Vec<_>>();
            if lis.len()<2{
                return None;
            }
        ...
        if let Some(input) = input{
            let count = super::super::utils::utils::remove_non_digit_chars(input).expect("Cannot parse to u32");
            return Some(count);
        }else {
            return None;
        }
