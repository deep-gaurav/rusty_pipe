...
impl YTTrendingExtractor{
    ...
    pub fn get_videos(&self)->Result<Vec<YTStreamInfoItemExtractor>,ParsingError>{
        let item_section_renderers = (||self.initial_data.get("contents")?
        ...
        for item_section in item_section_renderers{
            let shelf_content = (||item_section.get("itemSectionRenderer")?
                .get("contents")?
                .get(0)?
                .get("shelfRenderer")?
                .get("content")?
            ...
            if let Some(shelf_content)=shelf_content{
                for ul in shelf_content{
                    if let Some(videoRenderer) = ul.get("videoRenderer").unwrap_or(&Value::Null).as_object(){
                        videos.push(
