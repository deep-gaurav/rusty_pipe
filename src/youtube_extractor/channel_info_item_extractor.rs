...
use crate::utils::utils::{get_text_from_object, remove_non_digit_chars, mixed_number_word_parse};
...
impl YTChannelInfoItemExtractor {
    ...
    pub fn get_thumbnails(&self) -> Result<Vec<Thumbnail>,ParsingError> {
        ...
        for thumb in self.channel_info.get("thumbnail").ok_or("No thumbnail")?
    ...
    pub fn channel_id(&self) -> Result<String,ParsingError>{
        let channel_id  = self.channel_info.get("channelId").ok_or("Cant get playlist id")?.as_str().ok_or("Cant get playlist id")?;
    ...
    pub fn get_subscriber_count(&self) -> Result<i32,ParsingError> {
        if let Some(vct) = self.channel_info.get("subscriberCountText"){
            match get_text_from_object(vct,false){
                Ok(uploader) => {
                    Ok(
                        mixed_number_word_parse(&uploader.unwrap_or_default())
    ...
    pub fn get_stream_count(&self) -> Result<i32,ParsingError> {
        if let Some(vct) = self.channel_info.get("videoCountText"){
            match get_text_from_object(vct,false){
                Ok(uploader) => {
                    Ok(
                        remove_non_digit_chars::<i32>(&uploader.unwrap_or_default()).map_err(|e|ParsingError::from(e.to_string()))?
                    )
                },
                Err(err) => {
                    Err(err)
