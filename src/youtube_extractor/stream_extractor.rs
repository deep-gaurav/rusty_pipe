...
impl<D: Downloader> YTStreamExtractor<D> {
    ...
    fn get_itags(
        ...
        player_response: &Map<String, Value>,
        decryption_code: &str,
    ) -> Result<HashMap<String, Itag>, String> {
                                ...
                                if itag_item.itag_type == itag_type_wanted {
                                    ...
                                    let stream_url =
                                        match format_data.get("url").unwrap_or(&Value::Null) {
                                            ...
                                            _ => {
                                                let cipherstr = {
                                                    if let Value::String(cip) = format_data
                                                        .get("cipher")
                                                        .unwrap_or(&Value::Null)
                                                    {
                                                        cip.clone()
    ...
    pub async fn get_video_stream(&self) -> Result<Vec<StreamItem>, String> {
        ...
        for entry in YTStreamExtractor::<D>::get_itags(
            ...
            let itag = entry.1;
            video_streams.push(StreamItem { url: entry.0, itag });
        }
        Ok(video_streams)
    }
    ...
    pub async fn get_audio_streams(&self) -> Result<Vec<StreamItem>, String> {
        ...
        for entry in YTStreamExtractor::<D>::get_itags(
            ...
            let itag = entry.1;
            audio_streams.push(StreamItem { url: entry.0, itag });
        }
    ...
    async fn get_initial_data(url:&str,downloader:&D)->Result<(Value,bool),String>{
        ...
        headers.insert("X-YouTube-Client-Version".to_string(),HARDCODED_CLIENT_VERSION.to_string());
        ...
        let initial_ajax_json = initial_ajax_json.as_array().ok_or("inital ajax json not array")?;
        if let Some(initial_data)=initial_ajax_json.get(2).ok_or("inital ajax 2 not found")?.as_object(){
            if let Some(response)=initial_data.get("response"){
                Ok((response.clone(),true))
            }else{
                if let Some(initial_data) = initial_ajax_json.get(3).ok_or("initial ajax 2 not found")?.as_object(){
                    if let Some(response)=initial_data.get("response"){
...
impl<D: Downloader> YTStreamExtractor<D> {
    ...
    pub fn get_name(&self) -> Option<String> {
        ...
        let title = self.doc_html
            .select(&Selector::parse("meta[name=title]").unwrap())
            .next()?
            .value()
            .attr(CONTENT)?;
        Some(title.to_owned())
    ...
    pub fn get_video_thumbnails(&self)->Result<Vec<Thumbnail>,String>{
        if let Value::Object(video_details)=self.player_response.get("videoDetails").ok_or("cant get video Details")?{
            if let Value::Object(thumbnail) = video_details.get("thumbnail").ok_or("cant get thumbnail")?{
                if let Value::Array(thumbnails) = thumbnail.get("thumbnails").ok_or("Cant get thumbnails array")?
                {
                    let mut thumbnails_str = vec![];
                    for thumb in thumbnails{
                        let mut thumbnail:Thumbnail = serde_json::from_value(thumb.clone()).map_err(|e|e.to_string())?;
